...
export default function ResultadoQuizPage() {
  ...
  // Verificar se é o resultado detalhado (novo formato) ou EstudanteQuiz (formato antigo)
  const isResultadoDetalhado = tentativaAtual && 'quiz' in tentativaAtual && 'tentativa' in tentativaAtual && 'respostas' in tentativaAtual;
  
  // Se é resultado detalhado, já temos todos os dados
  // Se não, precisamos buscar o quiz separadamente (fallback para compatibilidade)
  const quizId = isResultadoDetalhado 
    ? (tentativaAtual as any).quiz.id 
    : tentativaAtual 
      ? (typeof (tentativaAtual as any).quiz === 'string' ? (tentativaAtual as any).quiz : (tentativaAtual as any).quiz?._id) 
      : undefined;
      
  const { quiz, isLoading: isLoadingQuiz } = useQuiz(isResultadoDetalhado ? undefined : quizId); // Só busca se não for resultado detalhado
  const [questoesAbertas, setQuestoesAbertas] = useState<Record<string, boolean>>({});
  console.log('=== DEBUG ESTRUTURA ===');
  console.log('isResultadoDetalhado:', isResultadoDetalhado);
  console.log('quizId extraído:', quizId);
  console.log('quiz separado (se necessário):', quiz);
  ...
  const getDadosTentativa = () => {
    if (isResultadoDetalhado) {
      ...
      return {
        ...
        tempoTotal: resultado.tentativa.duracao // duracao já vem em segundos
      };
    } else if (tentativaAtual) {
      const tentativa = tentativaAtual as any;
      return {
        id: tentativa._id,
        ...
        totalPontos: tentativa.totalPontos,
        respostasCorretas: tentativa.respostasCorretas,
  ...
  const isLoading = isLoadingTentativa || (isLoadingQuiz && !isResultadoDetalhado) || !tentativaAtual || (!dadosQuiz && !isResultadoDetalhado);
